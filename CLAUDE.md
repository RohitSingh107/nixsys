...
- **fedora/rohit/** — Mid-range, mirrors opensuse-hp: kitty, fish, tmux, starship, neovim, sway, waybar. ~9 modules. Fedora owns the compositor and its PAM-facing parts (`sway`, `swaymsg`/`swaynag`, `swaylock`, `Xwayland`, `pactl`, `nautilus`), so `sway.nix` keeps `package = null` and only writes `~/.config/sway/config`. Everything else in the session — waybar, wofi, mako, swayidle, grim, slurp, wl-clipboard, libnotify, playerctl, brightnessctl — comes from Nix. **The session PATH is `/usr/local/bin:/usr/bin` with no nix profile on it** (`systemctl --user show-environment`), so anything Nix provides must be referenced by absolute store path in these configs; a bare name silently resolves to Fedora's copy or to nothing. Other WM/shell modules are left out, as is `xdg.nix`, whose portal config would conflict with Fedora's.
